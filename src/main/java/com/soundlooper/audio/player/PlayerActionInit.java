...
import com.soundlooper.exception.PlayerRuntimeException;
...
public class PlayerActionInit extends PlayerAction {
	...
	public void run() throws PlayerException {
		try {
			...
			if (VERSIONS.NATIVEFMODEX_LIBRARY_VERSION != VERSIONS.NATIVEFMODEX_JAR_VERSION) {
				throw new PlayerException("Error!  NativeFmodEx library version ("
						+ VERSIONS.NATIVEFMODEX_LIBRARY_VERSION + ") is different to jar version ("
			...
			try {
				...
				throw new PlayerRuntimeException(
						"Une erreur est survenue à l'initialisation du lecteur. Une carte son est elle disponible?", e);
			}

			this.getPlayer().setState(PlayerState.STATE_PLAYER_INITIALIZED);
		} catch (InitException e) {
			throw new PlayerException("Erreur lors de l'initialisation de la librairie audio", e);
		}
	}
	...
	private void initializeSoundFormat(FMOD_SOUND_FORMAT fmodSoundFormatCelt) {
		try {
			Player.errorCheck(Player.getSystem().setSoftwareFormat(44100, fmodSoundFormatCelt, 0, 0,
					FMOD_DSP_RESAMPLER.FMOD_DSP_RESAMPLER_SPLINE));
		} catch (PlayerException e) {
			this.logger.error(e);
